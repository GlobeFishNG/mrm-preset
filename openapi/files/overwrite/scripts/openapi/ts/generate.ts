...
import { validateYaml, isSwagger, SesApiInfo, sesApiInfoJoiSchema, bundleYaml } from './openapi';
...
`/**
 * All functions were automatically generated by \`npm run openapi:codegen\`.
...
async function generateApiCode(yamlFile: string): Promise<void> {
  ...
  let xSesApiInfo = <SesApiInfo>_.get(oasApi, 'x-ses-api-info');

  const joiRet = sesApiInfoJoiSchema.validate(xSesApiInfo, {
    allowUnknown: true,
    convert: true,
  ...
  if (joiRet.error) {
    console.warn('--> API yaml doesn\'t include valid x-ses-api-info', joiRet.error.details[0].message);
    throw new Error('API yaml doesn\'t include valid x-ses-api-info');
  }

  xSesApiInfo = <SesApiInfo>joiRet.value;
  ...
  const appendInfo: AppendFileInfo = {
    header: '/* eslint-disable prefer-arrow/prefer-arrow-functions */',
  ...
  if (xSesApiInfo.network === 'internal-data-network') {
