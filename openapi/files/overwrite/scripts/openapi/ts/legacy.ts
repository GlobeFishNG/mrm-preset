...
// const joiImport = `import * as Joi from '@hapi/joi';\n`
...
async function processYaml(): Promise<void> {
  const yamlFiles = generateFileList('./docs/api/', '.yml');
  for (const yamlFile of yamlFiles) {
    ...
    if (bundledJsonSchemas) {
      shelljs.mkdir('-p', outputDir);
      const outputTsFile = path.join(outputDir, 'api-definitions.ts');
      await generateTs(bundledJsonSchemas, yamlFile, outputTsFile);
      console.log('--> Output Typescript interface file:', outputTsFile);
      // const outputJoiFile = path.join(outputDir, 'joi-schemas.ts');
